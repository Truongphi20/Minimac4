...
TEST(Haploid_run, compress)
{
    // Open file and redirect stdout
    FILE* fp = freopen("ref_panel_with_haploid_sample.msav", "w", stdout);

    // Create args string
    std::vector<std::string> compress_args{
        "minimac4",
        "--compress-reference", std::string(TEST_DATA) + "/ref_panel_with_haploid_sample.vcf.gz"
    };
